...
import * as snomedLib from '../lib/snomedv2';
...
router.get('/:db/:collection/concepts/:sctid/children?', async (req, res) => {
    const idParamStr = req.params.sctid;
    let query: any = { "relationships": { "$elemMatch": { "destination.conceptId": idParamStr, "type.conceptId": "116680003", "active": true } } };
    if (req.query["form"]) {
    ...
    let docs = await snomedLib.getObject(req.params.db, req.params.collection, query, options);
    res.status(200);
    if (!docs) docs = [];
    res.send(docs);
});
...
router.get('/:db/:collection/concepts', async (req, res) => {
    ...
    let docs = await snomedLib.getObject(req.params.db, req.params.collection, query, {});
    if (!docs) docs = [];
    res.status(200);
    res.send(docs);
});
...
router.get('/:db/:collection/concepts/:sctid/references?', async (req, res) => {
    const options = getOptions(req);
    let idParamStr = req.params.sctid;
    let query: any = { "relationships": { "$elemMatch": { "destination.conceptId": idParamStr, "active": true } } };
    let typeId = "900000000000011006";
...
router.get('/:db/:collection/concepts/:sctid/parents?', async (req, res) => {
    ...
    let docs = await snomedLib.getParents(req.params.db, req.params.collection, req.params.sctid, req.query["form"], options);
    if (!docs) docs = [];
    res.status(200);
    res.send(docs);
});
...
router.get('/:db/:collection/descriptions/:sctid?', async (req, res) => {
    let idParamStr = null;
    let query: any = { 'descriptions.descriptionId': 0 };
    ...
    let semanticFilter = "none";
    let moduleFilter = "none";
    let langFilter = "none";
    let refsetFilter = "none";
    let statusFilter;
    let returnLimit = 100;
    let skipTo = 0;
    if (req.params.sctid) {
